        ...
        try:
            if 'name' in notification_category_dict and notification_category_dic['name'] is not None:
                ...
                if NotificationCategory.is_name_unique(id=id, name=notification_category_name):
                    ...
                    response = {
                        'error': 'A category with the name {} already exists'.format(notification_category_name)}
                    return response, HttpStatus.bad_request_400.value
...
class NotificationCategoryListResource(AuthenticationRequiredResource):
    ...
    def post(self):
        ...
        if not NotificationCategory.is_name_unique(id=0, name=notification_category_name):
            response = {
                'error': 'A notification category with the name {} already exists'.format(notification_category_name)}
            return response, HttpStatus.bad_request_400.value
        try:
